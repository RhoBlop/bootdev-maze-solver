...
class TestMaze(unittest.TestCase):
    def test_maze_create_cells_1(self):
        ...
        """
        maze = Maze(num_rows=2, num_cols=2, seed=RANDOM_SEED)
    ...
    def test_maze_create_cells_2(self):
        ...
        """
        maze = Maze(num_rows=3, num_cols=2, seed=RANDOM_SEED)
        num_rows = len(maze.get_cells())
        num_cols = len(maze.get_cells()[0])
    ...
    def test_maze_create_cells_3(self):
        ...
        """
        maze = Maze(num_rows=2, num_cols=3, seed=RANDOM_SEED)
        num_rows = len(maze.get_cells())
        num_cols = len(maze.get_cells()[0])
    ...
    def test_maze_open_entrance_and_exit(self):
        """
        Should open (set to False) both a left wall on top-left cell and a right wall on bottom-right cell 
        """
        maze = Maze(num_rows=2, num_cols=2, seed=RANDOM_SEED)
        maze._open_entrance_and_exit()
        cells = maze.get_cells()

        # left
        self.assertFalse(cells[0][0].get_walls()[3], "Entrance wall not open")
        #right
        self.assertFalse(cells[-1][-1].get_walls()[1], "Exit wall not open")
    ...
    def test_maze_neighbours_corners(self):
        ...
        """
        nr, nc = 4, 4
        maze = Maze(num_rows=nr, num_cols=nc, seed=RANDOM_SEED)
        cells = maze.get_cells()
